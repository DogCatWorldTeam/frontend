...
function Form() {
  ...
  // 이미지 리스트 미리보기
  const imgListHandler = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
      ...
      <FromHeader>
        ...
        <InfoContainer>
          ...
          <InfoDetail>
            ...
            <InputBtnContainer>
              <InputGenderBtn type="button" gender={isGender === 'male'} onClick={() => genderBtnHendler('male')}>남</InputGenderBtn>
              <InputGenderBtn type="button" gender={isGender === 'female'} onClick={() => genderBtnHendler('female')}>여</InputGenderBtn>
            </InputBtnContainer>
            <InputInfo type="text" placeholder="나이" />
            <InputInfo type="text" placeholder="몸무게" />
            <InputInfo type="text" placeholder="품종" />
            <InputBtnContainer>
              <InputNeuterBtn type="button" neuter={isNeuter === 'yes'} onClick={() => neuterBtnHandler('yes')}>네</InputNeuterBtn>
              <InputNeuterBtn type="button" neuter={isNeuter === 'no'} onClick={() => neuterBtnHandler('no')}>아니요</InputNeuterBtn>
            </InputBtnContainer>
      ...
      <InputImgList>
        {[...Array(4)].map((_, index) => (
          <ImgPreviewList key={`listImg${index}`} htmlFor={`listImg${index}`}>
            ...
            <InputFile type="file" id={`listImg${index}`} onChange={(e) => imgListHandler(e, index)} />
          </ImgPreviewList>
        ))}
      </InputImgList>
